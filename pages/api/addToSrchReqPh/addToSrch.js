...
const addToSrch = async(req, res) => {
    const { term } = req.body
    const srchArr = []
    const inData = fs.readFileSync('/public/search_require_photos.json')
    ...
    fs.writeFileSync('/public/search_require_photos.json', JSON.stringify(addingData), () => {})
...
const clearAllSrches = async(req, res) => {
    ...
    const addingData = []
        // console.log("the finale data is ",addingData)
    fs.writeFileSync('/public/recent_searches.json', JSON.stringify(addingData), () => {})
...
const getAllReqPhSrches = async(req, res) => {
    const searches = []
    const data = fs.readFileSync('/public/search_require_photos.json')
